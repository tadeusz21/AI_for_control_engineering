...
for neuron_nb=neurons_list
    ...
    mkdir("../oe_models/" + "oe_neurons=" + string(neuron_nb));
    train_errors = zeros(attempts, 1);
    val_errors = zeros(attempts, 1);

    for attempt=1:attempts
        mkdir("../oe_models/" + "oe_neurons=" + string(neuron_nb) + "/attempt=" + string(attempt));
        system("sieci.exe");
        ...
        movefile("uczenie.m", "../oe_models/" + "oe_neurons=" + string(neuron_nb) + "/attempt=" + string(attempt) + "/uczenie.m");
        movefile("model.m", "../oe_models/" + "oe_neurons=" + string(neuron_nb) + "/attempt=" + string(attempt) + "/model.m");
        train_errors(attempt) = train_error;
        val_errors(attempt) = val_error;
        save("../oe_models/" + "oe_neurons=" + string(neuron_nb) + "/attempt=" + string(attempt) + "/simulations.mat", 'y_pred_train', 'y_pred_val');
    end
    save("../oe_models/" + "oe_neurons=" + string(neuron_nb) + "/errors.mat", 'train_errors', 'val_errors');

    % Write which model is the best in terms of error on validation dataset
    fileID = fopen("../oe_models/" + "oe_neurons=" + string(neuron_nb) + "/best_model.txt"', 'w');
